...
/* stall count */
static counter_t stall_count = 0;
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
    ...
    stat_reg_counter(sdb, "stall_count",
            "total number of cycless stalling",
            &stall_count, 0, NULL);
...
#include "machine.def"
    ...
    /* buffer for last cycle sim_total_insn */
    counter_t last_sim_total_insn = sim_total_insn;
    ...
    for (;;)
    {
        ...
        /* if the program stalls, increase the stall count. */
        if(last_sim_total_insn == sim_total_insn){
            stall_count++;
        }
